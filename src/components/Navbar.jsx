      ...
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        >
          ...
          <p className='text-white text-[18px] flex font-bold cursor-pointer'>
            Rohit &nbsp; <span className='sm:block hidden'>| Web Developer</span>
          </p>
        </Link>
            ...
            <ul className='list-none flex justify-end items-start flex-col gap-4'>
              {
                navLinks.map((e) => {
                  return <li key={e.id}
                    className={`${active === e.title ? 'text-white' : 'text-secondary'} font-poppins font-medium text-[16px] cursor-pointer `}
                    onClick={() => { setToggle(!toggle); setActive(e.title) }} >
                    <a href={`#${e.id}`}>{e.title}</a>
                  </li>
                })
              }
